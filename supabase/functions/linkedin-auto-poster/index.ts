// supabase/functions/linkedin-auto-poster/index.ts
...
// --- Environment Variable for the linkedin-poster Edge Function URL ---
// This Edge Function needs to know the public URL of the linkedin-poster function it triggers
const LINKEDIN_POSTER_EDGE_FUNCTION_URL = Deno.env.get('LINKEDIN_POSTER_EDGE_FUNCTION_URL');

if (!LINKEDIN_POSTER_EDGE_FUNCTION_URL) {
    console.error('Edge Function: Missing LINKEDIN_POSTER_EDGE_FUNCTION_URL environment variable.');
...
serve(async (req) => {
  try {
    console.log("LinkedIn auto-poster function started");
...
      .eq('social_channel', 'LinkedIn'); // <--- Filter for LinkedIn posts
...
        // (Or you might have a separate 'profiles' or 'users' table linked by user_id)
         // Let's assume timezone is on social_channels table and linked by user_id
...
             .in('user_id', uniqueUserIds) // Filter for only users who have candidate posts
             .eq('social_channel', 'LinkedIn'); // <--- Important: Get timezone from their LinkedIn account record
...
    // --- Step 3: Trigger linkedin-poster Edge Function for each filtered post ---
    let processedCount = 0;
    const failedToProcess = []; // Track posts that failed to trigger the poster function

    console.log(`Found ${postsToProcess.length} posts passing all criteria including timezone-aware time window. Triggering linkedin-poster for each...`);

     if (!LINKEDIN_POSTER_EDGE_FUNCTION_URL) {
          console.error('Edge Function: Cannot trigger linkedin-poster, URL is missing.');
          // Return a fatal error since the core task cannot be performed
           return new Response(JSON.stringify({
             error: 'LinkedIn poster Edge Function URL not configured',
...
      try {
        console.log(`Attempting to trigger linkedin-poster for post ID: ${post.id}`);

        // --- Trigger the linkedin-poster Edge Function via HTTP POST ---
        const triggerResponse = await fetch(LINKEDIN_POSTER_EDGE_FUNCTION_URL, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                 // No Authorization header needed here if linkedin-poster uses Service Role Key
            },
        ...
        // Check the response from the linkedin-poster Edge Function
        if (!triggerResponse.ok) {
            const errorBody = await triggerResponse.json().catch(() => null); // Attempt to parse error body
            console.error(`Error triggering linkedin-poster for post ${post.id}:`, triggerResponse.status, triggerResponse.statusText, 'Body:', errorBody);
            ...
            // await supabase.from('user_post_schedule').update({ schedule_status: false, sent_post: false, error_message: `Failed to trigger poster: ${triggerResponse.status}` }).eq('id', post.id);
             ...
             // Assuming linkedin-poster returns 200 on success or handled duplicate
            console.log(`Successfully triggered linkedin-poster for post ${post.id}`);
             processedCount++; // Increment count if triggering was successful
             // The actual sent_post/schedule_status update is handled BY linkedin-poster
        }

      } catch (triggerError) {
        console.error(`Network or unhandled error triggering linkedin-poster for post ${post.id}:`, triggerError);
...
    return new Response(JSON.stringify({
      message: 'LinkedIn auto-poster function completed',
...
  } catch (error) {
    console.error('Fatal error in linkedin auto-poster function:', error);
    return new Response(JSON.stringify({
